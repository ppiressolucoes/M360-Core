...
final class M360_Search_Controller
{
    ...
    public static function render(): string
    {
        ...
        ob_start();
        echo '<main class="m360-dynamic-view m360-search-view">';
        echo '<div class="m360-dynamic-view__container">';
        echo do_shortcode('[m360_breadcrumb]');
        echo '<header class="m360-search-view__header">';
        echo '<span class="m360-search-view__eyebrow">' . esc_html($is_en ? 'Mengão 360 Search' : 'Busca Mengão 360') . '</span>';
        echo '<h1>' . esc_html($title) . '</h1>';
        echo '<p>' . esc_html($is_en ? 'Search term:' : 'Termo pesquisado:') . ' <strong>' . esc_html($query) . '</strong></p>';
        echo '</header>';
        ...
        if (have_posts()) {
            ...
            echo '<section class="m360-search-empty">';
            echo '<h2>' . esc_html($empty_title) . '</h2>';
    ...
    private static function render_card(): string
    {
        ...
        if ($excerpt === '') {
            $excerpt = wp_trim_words(wp_strip_all_tags((string) get_the_content()), 26);
        }

        ob_start();
        echo '<article class="m360-search-card">';
        if (has_post_thumbnail()) {
            echo '<a class="m360-search-card__thumb" href="' . esc_url(get_permalink()) . '">';
            the_post_thumbnail('medium_large');
            echo '</a>';
        }
        echo '<div class="m360-search-card__body">';
        if ($category !== '') {
            echo '<span class="m360-search-card__category">' . esc_html($category) . '</span>';
        }
    ...
    {
        if (wp_style_is('m360-core-search', 'registered')) {
            wp_enqueue_style('m360-core-search');
        }
