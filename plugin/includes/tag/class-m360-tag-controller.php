...
final class M360_Tag_Controller
{
    ...
    public static function render(): string
    {
        ...
        ob_start();
        echo '<main class="m360-dynamic-view m360-tag-view">';
        echo '<div class="m360-dynamic-view__container">';
        echo do_shortcode('[m360_breadcrumb]');
        echo '<section class="m360-tag-hero">';
        echo '<span class="m360-tag-hero__eyebrow">' . esc_html($is_en ? 'Mengão 360 Tag' : 'Tag Mengão 360') . '</span>';
        ...
        echo '<div class="m360-tag-stats"><span><strong>' . esc_html((string) $term->count) . '</strong> ' . esc_html($count_label) . '</span></div>';
        echo '</section>';

        if (have_posts()) {
            echo '<section class="m360-tag-results" aria-label="' . esc_attr($is_en ? 'Tag articles' : 'Artigos da tag') . '">';
            while (have_posts()) { the_post(); echo self::render_card($term); }
            echo '</section>';
            echo self::pagination();
        } else {
            echo '<section class="m360-tag-empty"><h2>' . esc_html($is_en ? 'No articles found' : 'Nenhum artigo encontrado') . '</h2></section>';
        }
    ...
    private static function render_card(WP_Term $term): string
    {
        ...
        $category = !empty($categories) ? $categories[0]->name : $term->name;
        $excerpt = get_the_excerpt();
        if ($excerpt === '') { $excerpt = wp_trim_words(wp_strip_all_tags((string) get_the_content()), 26); }

        ob_start();
        echo '<article class="m360-tag-card">';
        if (has_post_thumbnail()) {
            echo '<a class="m360-tag-card__thumb" href="' . esc_url(get_permalink()) . '">';
            the_post_thumbnail('medium_large');
            echo '</a>';
        }
        echo '<div class="m360-tag-card__body">';
        echo '<span class="m360-tag-card__category">' . esc_html($category) . '</span>';
