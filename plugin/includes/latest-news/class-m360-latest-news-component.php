...
final class M360_Latest_News_Component
{
    ...
    public static function render(array $args = []): string
    {
        ...
        $cache_key = 'm360_latest_news_' . md5(wp_json_encode([$limit, $is_en, $layout, $show_image, $show_category, $show_date]));
        if ($cache_enabled) {
    ...
    private static function render_item(int $index, bool $show_image, bool $show_category, bool $show_date): string
    {
        ...
        if ($show_image && has_post_thumbnail()) {
            ...
            the_post_thumbnail($index === 1 ? 'medium_large' : 'thumbnail', ['loading' => 'lazy']);
            echo '</a>';
        }
        echo '<div class="m360-latest-news__body">';
        if ($show_category && $category instanceof WP_Term) {
            echo '<a class="m360-latest-news__category" href="' . esc_url(get_category_link($category)) . '">' . esc_html($category->name) . '</a>';
        }
        echo '<h3 class="m360-latest-news__title"><a href="' . esc_url(get_permalink()) . '">' . esc_html(get_the_title()) . '</a></h3>';
        if ($show_date) {
            echo '<time class="m360-latest-news__date" datetime="' . esc_attr(get_the_date('c')) . '">' . esc_html(human_time_diff(get_the_time('U'), current_time('timestamp')) . ' ' . (self::is_en() ? 'ago' : 'atrás')) . '</time>';
        }
        echo '</div></article>';
...
if (!function_exists('m360_latest_news')) {
    function m360_latest_news(array $args = []): string
    {
        return M360_Latest_News_Component::render($args);
    }
}
