...
final class M360_Ads_Admin
{
    ...
    public static function render_dashboard(): void
    {
        ...
        echo '<div class="wrap m360-ads-admin"><h1>M360 Ads Manager</h1><p>Gestao inicial de inventario publicitario do Mengao 360.</p>';
        echo '<div class="m360-ads-admin__cards">';
        self::metric('Slots', $total_slots);
        self::metric('Slots ativos', $active_slots);
        self::metric('Slots ocupados', $assigned);
        self::metric('Campanhas', $total_campaigns);
        self::metric('Campanhas ativas', $active_campaigns);
        self::metric('Schema Ads', esc_html((string) get_option('m360_ads_db_version', '-')));
        echo '</div>';
        echo '<p><a class="button button-primary" href="' . esc_url(admin_url('admin.php?page=m360-ads-campaign-new')) . '">Nova campanha</a> ';
        echo '<a class="button" href="' . esc_url(admin_url('admin.php?page=m360-ads-slots')) . '">Ver slots</a></p></div>';
    }
    ...
    public static function render_slots(): void
    {
        ...
        foreach ($rows as $row) {
            ...
            echo '<tr><td><strong>' . esc_html($row['name']) . '</strong><br><code>' . esc_html($row['slot_key']) . '</code></td>';
            echo '<td>' . esc_html($row['page_context']) . '</td><td>' . esc_html($row['language']) . '</td><td>' . esc_html($row['device']) . '</td>';
            echo '<td>' . esc_html((string) $row['max_width']) . 'x' . esc_html((string) $row['max_height']) . '</td>';
            echo '<td><code>[m360_ad_slot id=&quot;' . esc_html($row['slot_key']) . '&quot;]</code></td><td>';
            echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '">';
            wp_nonce_field('m360_ads_assign_campaign');
            echo '<input type="hidden" name="action" value="m360_ads_assign_campaign"><input type="hidden" name="slot_id" value="' . esc_attr((string) $row['id']) . '">';
            echo '<select name="campaign_id"><option value="0">Nenhuma</option>';
            foreach ($active_campaigns as $campaign) {
                echo '<option value="' . esc_attr((string) $campaign['id']) . '"' . selected($current, (int) $campaign['id'], false) . '>' . esc_html($campaign['title']) . '</option>';
            }
    ...
    public static function render_campaigns(): void
    {
        self::guard();
        global $wpdb;
        $table = M360_Ads_DB::table('ad_campaigns');
        ...
        foreach ($rows as $row) {
            $edit = admin_url('admin.php?page=m360-ads-campaign-new&id=' . (int) $row['id']);
            $delete = wp_nonce_url(admin_url('admin-post.php?action=m360_ads_delete_campaign&id=' . (int) $row['id']), 'm360_ads_delete_campaign');
            echo '<tr><td><strong>' . esc_html($row['title']) . '</strong></td><td>' . esc_html($row['advertiser']) . '</td><td>' . esc_html($row['campaign_type']) . '</td><td>' . esc_html($row['language']) . '</td><td>' . esc_html($row['device']) . '</td><td>' . esc_html($row['status']) . '</td><td>' . esc_html((string) $row['priority']) . '</td><td><a href="' . esc_url($edit) . '">Editar</a> | <a href="' . esc_url($delete) . '" onclick="return confirm(\'Excluir campanha?\')">Excluir</a></td></tr>';
        }
    ...
    public static function render_campaign_form(): void
    {
        ...
        echo '<div class="wrap m360-ads-admin"><h1>' . ($id ? 'Editar campanha' : 'Nova campanha') . '</h1><form method="post" action="' . esc_url(admin_url('admin-post.php')) . '">';
        wp_nonce_field('m360_ads_save_campaign');
        echo '<input type="hidden" name="action" value="m360_ads_save_campaign"><input type="hidden" name="id" value="' . esc_attr((string) $id) . '">';
        echo '<table class="form-table"><tbody>';
        self::input('Titulo', 'title', $row['title'], true);
        self::input('Anunciante', 'advertiser', $row['advertiser']);
        self::select('Tipo', 'campaign_type', $row['campaign_type'], ['image'=>'image','html'=>'html','adsense'=>'adsense','gam'=>'gam','house'=>'house','affiliate'=>'affiliate','sponsor'=>'sponsor']);
        self::input('Imagem URL', 'image_url', $row['image_url']);
        ...
        self::input('Inicio', 'start_at', $row['start_at']);
        self::input('Fim', 'end_at', $row['end_at']);
        self::input('Prioridade', 'priority', (string) $row['priority']);
        self::select('Status', 'status', $row['status'], ['draft'=>'draft','active'=>'active','paused'=>'paused','expired'=>'expired','archived'=>'archived']);
    ...
    {
        self::guard();
        check_admin_referer('m360_ads_save_campaign');
        global $wpdb;
        ...
        $data = [
            ...
            'advertiser' => sanitize_text_field((string) ($_POST['advertiser'] ?? '')),
            'campaign_type' => sanitize_key((string) ($_POST['campaign_type'] ?? 'image')),
            ...
            'start_at' => self::nullable_datetime((string) ($_POST['start_at'] ?? '')),
            'end_at' => self::nullable_datetime((string) ($_POST['end_at'] ?? '')),
        ...
        wp_safe_redirect(admin_url('admin.php?page=m360-ads-campaigns'));
        exit;
    }
    ...
    {
        self::guard();
        check_admin_referer('m360_ads_delete_campaign');
        global $wpdb;
        $id = absint($_GET['id'] ?? 0);
        if ($id) {
            $wpdb->delete(M360_Ads_DB::table('ad_slot_campaigns'), ['campaign_id' => $id]);
            $wpdb->delete(M360_Ads_DB::table('ad_campaigns'), ['id' => $id]);
        }
        wp_safe_redirect(admin_url('admin.php?page=m360-ads-campaigns'));
        exit;
    }
    ...
    {
        self::guard();
        check_admin_referer('m360_ads_assign_campaign');
        global $wpdb;
        $slot_id = absint($_POST['slot_id'] ?? 0);
        $campaign_id = absint($_POST['campaign_id'] ?? 0);
        $table = M360_Ads_DB::table('ad_slot_campaigns');
        if ($slot_id) {
            $wpdb->update($table, ['is_active' => 0, 'updated_at' => current_time('mysql')], ['slot_id' => $slot_id]);
            if ($campaign_id) {
        ...
        wp_safe_redirect(admin_url('admin.php?page=m360-ads-slots'));
        exit;
    }
