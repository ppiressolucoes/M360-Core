...
final class M360_Ads_Creatives_Admin
{
    ...
    private static function media_input(string $label, string $name, array $row): void { $value = (string) ($row[$name] ?? ''); echo '<tr><th><label for="' . esc_attr($name) . '">' . esc_html($label) . '</label></th><td><input class="regular-text m360-ads-media-url" id="' . esc_attr($name) . '" name="' . esc_attr($name) . '" value="' . esc_attr($value) . '"> <button type="button" class="button m360-ads-media-pick">Selecionar imagem</button><div class="m360-ads-media-preview">' . ($value ? '<img src="' . esc_url($value) . '">' : '') . '</div>' . self::media_meta_panel($row) . '</td></tr>'; }
    private static function media_meta_panel(array $row): string { $file = !empty($row['image_url']) ? wp_basename((string) $row['image_url']) : '-'; $size = !empty($row['width']) && !empty($row['height']) ? $row['width'] . 'x' . $row['height'] : '-'; $bytes = !empty($row['filesize']) ? size_format((int) $row['filesize']) : '-'; $mime = !empty($row['mime']) ? (string) $row['mime'] : '-'; return '<div class="m360-ads-media-meta"><strong>Dados do criativo</strong><dl><dt>Arquivo</dt><dd data-m360-media="file">' . esc_html($file) . '</dd><dt>Formato</dt><dd data-m360-media="size">' . esc_html($size) . '</dd><dt>Peso</dt><dd data-m360-media="filesize">' . esc_html($bytes) . '</dd><dt>Tipo</dt><dd data-m360-media="mime">' . esc_html($mime) . '</dd><dt>Origem</dt><dd>Media Library</dd></dl><p class="m360-ads-size-warning" hidden>Este criativo não corresponde ao formato selecionado.</p></div>'; }
